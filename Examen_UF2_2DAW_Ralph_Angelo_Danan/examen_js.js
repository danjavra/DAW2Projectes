...
function prints(caracol,mascota, i) {
    ...
    return `
        <li class="list-group-item col-xs-6">
            <div class='col-xs-12'>
                ...
                <span class="col-xs-6">Mascota: ${mascota.nombre}</span>
                <span class="col-xs-6">Mascota: ${mascota.slogan}</span>
            </div>
        </li>
        `;
}
...
function sprint(){
   ...
   sprint.innerHTML+="<div class='col-xs-12' style='background-color: #8a6d3b; text-align: center'>SPRINT "+i+"</div>";
    ...
    for(k=0; k<Caracol.length;k++){
      ...
      sprint.innerHTML+="<table class='col-sm-6'><tr class='col-xs-12' style='background-color:#c0c0c0; border:5px solid white;'><td class='col-sm-1'>"+(k+1)+"º "+"</td><td class='col-sm-1'>"+"Nom: "+Caracol[k].nombre+"</td><td class='col-sm-1'>"+"Total: <span class='springtotal-"+i+"'>"+Caracol[k].distancia+"</span></td><td class='col-sm-1'>"+"Sprint: "+i+" Mascota: "+Mascota[k].nombre+" "+Mascota[k].slogan+" </td></tr></table>";
